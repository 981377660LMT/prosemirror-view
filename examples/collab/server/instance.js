...
class Instance {
  ...
  collectUsers() {
    ...
    this.userCount = 0
    this.collecting = null
    for (let i = 0; i < this.waiting.length; i++) this._registerUser(this.waiting[i].ip)
...
function doSave() {
  ...
  for (var prop in instances)
    out[prop] = { doc: instances[prop].doc.toJSON(), comments: instances[prop].comments.comments }
