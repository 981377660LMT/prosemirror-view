...
function populateDefaultInstances(newInstance) {
  ...
  newInstance(
    ...
    $node('doc', null, [
      $node('blockquote', null, [
        $node('paragraph', null, [
          $text('Mona tried to tell me'),
          $node('hard_break'),
          $text('To stay away from the train line.'),
          $node('hard_break'),
          $text('She said that all the railroad men'),
          $node('hard_break'),
          $text('Just drink up your blood like wine.'),
          $node('hard_break'),
          $text("An' I said, “Oh, I didn't know that,"),
          $node('hard_break'),
          $text("But then again, there's only one I've met"),
          $node('hard_break'),
          $text("An' he just smoked my eyelids"),
          $node('hard_break'),
          $text("An' punched my cigarette.”"),
          $node('hard_break'),
          $text('Oh, Mama, can this really be the end,'),
          $node('hard_break'),
          $text('To be stuck inside of Mobile'),
          $node('hard_break'),
  ...
  newInstance(
    ...
    $node('doc', null, [
      ...
      $node('paragraph', null, [
        $text(
          "If you look at it rationally you'll see that open-source software is communism and communism is bad because the free market is our God and savior. Also sheep caused 9/11."
        )
      ])
    ])
  )
